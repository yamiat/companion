...
interface YamiatEvent {
  message: string;
}

type USE_ITEM = "dnd5e.useItem";
...
const byId = (needle: any) => (item: any) => item.flags?.yamiat?.entity === needle.flags?.yamiat?.entity;
...
const fetchExport = async (exportId: string) => {
  const g = game as Game;
  const token = g.settings.get("yamiat-companion", "apiToken") as string;
  const url = g.settings.get("yamiat-companion", "apiUrl") as string;
...
const getCollection = async (name: string, label: string, type: any) => {
  const g = game as Game;
  // @ts-ignore
...
const d = new Dialog({
  ...
  buttons: {
   one: {
    ...
    callback: async (html) => {
      const g = game as Game;
      const exportId = $(html).find('#exportId').val() as string;
      const response = await fetchExport(exportId) as any;
      ...
      await Promise.all(sources.map(async (source) => {
        ...
        const items = await getCollection(
          `yamiat-${source.id}-items`,
          `${shortenName(source.name)} Items`,
          "Item"
        );
        const actors = await getCollection(
          `yamiat-${source.id}-actors`,
          `${shortenName(source.name)} Actors`,
          "Actor"
        );
        const scenes = await getCollection(
          `yamiat-${source.id}-scenes`,
          `${shortenName(source.name)} Scenes`,
          "Scene"
          );
          const journals = await getCollection(
            `yamiat-${source.id}-journals`,
            `${shortenName(source.name)} Journals`,
            "JournalEntry"
          );
      }));
      ...
      await Promise.all(data.actors.map(async (actor) => {
        ...
        if (existingActor) {
          ...
          await Actor.create(actor as any, {
            pack: actorsPath
          });
        }
      }))
      ...
      await Promise.all(data.scenes.map(async (scene) => {
        ...
        if (existingScene) {
          ...
          await Scene.create(scene as any, {
            pack: scenesPath
          });
        }
      }))
      ...
      await Promise.all(data.journals.map(async (journal) => {
        ...
        if (existingJournal) {
          ...
          await JournalEntry.create(journal as any, {
            pack: journalsPath
          });
        }
      }))
 ...
 Hooks.on("getSceneControlButtons", (controls: SceneControl[], b: any, c: any) => {
...
Hooks.on("createActor", (document:any, change:any, options:any, userId:any) => {
  console.log({ document, change, options, userId});
})

Hooks.on("updateActor", (document:any, change:any, options:any, userId:any) => {
  console.log({ document, change, options, userId});
  addEvent({date: new Date().toTimeString(), type: "updateActor", document, change, options, userId});
})

Hooks.on("createItem", (document:any, change:any, options:any, userId:any) => {
  console.log({ document, change, options, userId});
})

Hooks.on("updateItem", (document:any, change:any, options:any, userId:any) => {
  console.log({ document, change, options, userId});
  addEvent({date: new Date().toTimeString(), type: "updateItem", document, change, options, userId});
})

Hooks.on('targetToken', async (item: any, options: any) => {});

// dnd5e hooks

Hooks.on('dnd5e.preRollAttack', async (item: any, options: any) => {});

Hooks.on('dnd5e.rollAttack', async (item: Item, roll: Roll) => {
  console.log("Yamiat Companion: rollAttack", item, roll);
});

Hooks.on('dnd5e.preRollDamage', async (item: any, actor: any) => {});

Hooks.on('dnd5e.rollDamage', async (item: Item, roll: Roll) => {
  console.log("Yamiat Companion: rollDamage", item, roll);
});

Hooks.on('updateActor', async (actor: any, doc: any) => {
  console.log("Yamiat Companion: updateActor", actor, doc);
});

Hooks.on('updateToken', async (token: any, pos: {x: number, y: number}) => {
  console.log("Yamiat Companion: updateToken", token, pos);
});

Hooks.on('dnd5e.useItem', async () => {});

Hooks.on('dnd5e.itemUsageConsumption', async (item: any, config: any, options: any, usage: any) => {
...
Hooks.on('modifyTokenAttribute', async (item: any) => {});
